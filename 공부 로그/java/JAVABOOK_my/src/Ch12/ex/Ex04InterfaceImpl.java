...
public class Ex04InterfaceImpl {
    public static void main(String[] args) {
        ...
        piano.tune();
        Playable pPiano = piano;
        pPiano.play();
        // pPiano.tune(); - upcasting 상황이라 불가
        if(pPiano instanceof Piano) {
        	((Piano) pPiano).tune();
        }
        Instrument insPiano = piano;
        insPiano.play();
        // insPiano.tune(); - upcasting 상황이라 불가
        if(insPiano instanceof Piano) {
        	((Piano) insPiano).tune();
        }
