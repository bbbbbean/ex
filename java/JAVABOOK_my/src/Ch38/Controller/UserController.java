...
public class UserController implements SubController {
	...
	//				response.put("message","유효성 체크 오류");
				...
				// 03 관련 서비스 실행
				boolean isSuccess = userService.UserJoin(userDto);	// 서비스 성공 유무 전달
																	// 예외 관리 해줘야함, 던지지 말기 : 서비스-Dao연결되어 있음 -> 
	...
	// 예외 처리 함수 - 서비스를 처리하는 과정의
	public Map<String,Object> exceptionHandler(Exception e){
		// 컨트롤러 마다 처리 or 전역 처리
		// 여기서는 컨트롤러마다 처리
		// 예외 처리를 중앙으로 몰아주어야 해서 이렇게 처리
		// 어떤 예외든 다 받아 정보를 함께 전달하기 위한 함수
		
		// response에 예외 객체를 띄워 전달
		// response 객체가 없을 수도 있으니까 조건 걸고 만들어주기
